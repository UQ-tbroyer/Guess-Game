...
public class ClientHandler implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            if (playerName == null) {
                logger.logEvent("ClientHandler : timeout d'authentification pour " + clientIp + ". Connexion fermée.");
            } else {
                logger.logEvent("ClientHandler : timeout d'inactivité pour " + playerName + ". Connexion fermée.");
            }
        } catch (IOException e) {
            if (playerName != null) {
    ...
    String gameStartedMsg = "GG|GAME_STARTED|" + roomName + "|" + room.getMaxAttempts() + "|" + room.getAdminName() + "|" + payload;
    ...
    for (String pName : room.getPlayerNames()) {
        ...
        broadcastToRoom(room, MessageParser.serialize(CommandType.INFO,
                "ADMIN: après GAME_STARTED, tapez 'secret c1 c2 c3 c4' (vous ne jouerez pas) ou 'secret' (aléatoire, vous jouerez aussi)."));
