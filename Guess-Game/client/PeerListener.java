...
public class PeerListener extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            while ((rawLine = in.readLine()) != null) {
                ...
                handleRawLine(security.stripSignature(rawLine).trim());
            }
    ...
    private void handleRawLine(String rawLine) {
        ...
        switch (msg.getCommand()) {
            ...
            default -> logger.logError(
                    "PeerListener : commande non gérée : " + msg.getCommand());
        }
    }
    ...
    private void onGuess(Message msg) {
        ...
        if (guesser == null) {
            ...
            if (guesser != null) peerName = guesser;
        }
        if (guesser == null) guesser = "inconnu";
        ...
        try {
            ...
            boolean lastAttempt = msg.getFields().size() > GameEngine.COMBINATION_SIZE
                    && "LAST".equalsIgnoreCase(msg.getField(GameEngine.COMBINATION_SIZE));
            ...
            if (!feedback.isWin()) {
                if (lastAttempt) {
                    ...
                    logger.logEvent("PeerListener : " + guesser + " éliminé après son dernier essai.");
    ...
    private void onFeedback(Message msg) {
        // Le détenteur du secret ne doit pas voir le feedback des autres joueurs
        if (gameEngine.isSecretOwner()) {
        ...
        try {
            ...
            logger.logEvent("Feedback recu -> Couleurs OK : " + correctColors
            ...
            if (feedback.isWin()) {
                ...
                gameEngine.setGameOver(true);
                logger.logEvent("PeerListener : marque la manche comme terminee (WINNER).");
            } else {
                System.out.println("[GAME] Feedback : " + correctColors + " couleur(s) correcte(s), "
    ...
    private void onGameOver(Message msg) {
        try {
            String[] fields = msg.getFields().toArray(String[]::new);
    ...
    /**
     * GG|NEW_GAME — réinitialise l'état local complet via P2PManager.
     */
    private void onNewGame(@SuppressWarnings("unused") Message msg) {
    ...
    private void onTurnAnnouncement(Message msg) {
        try {
            ...
            if (currentPlayer.equals(p2pManager.getPlayerName())) {
                System.out.println("[GAME] La partie commence ! C'est votre tour de deviner.");
            } else {
                System.out.println("[GAME] La partie commence ! C'est le tour de " + currentPlayer + " de débuter.");
            }
     ...
     * Notifie tous les joueurs qu'un pair a épuisé ses tentatives.
     */
    private void onPlayerOut(Message msg) {
        try {
            ...
            p2pManager.decrementActivePlayers();
            System.out.println("[GAME] " + outPlayer + " a épuisé toutes ses tentatives !");
            logger.logEvent("PeerListener : " + outPlayer + " éliminé. Joueurs actifs : "
                    + p2pManager.getActivePlayersCount());
        } catch (ParseException e) {
